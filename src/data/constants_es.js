const nav = {
  about: "Bio",
  skills: "Habilidades",
  experience: "Experiencia",
  projects: "Proyectos",
  education: "Educación",
  contact: "Contacto",
  github: " Github Perfil"
};

const hero = {
  heroTittle: "Hola, soy",
  heroPresentation: "Soy un ",
  heroButton: "Visitar LinkedIn",
};

const bio = {
  name: "Matias Perdomo",
  roles1: "Full Stack Developer",
  roles2:"Android Developer",
  roles3:"Programador",
  description:
    "¡Me apasiona la tecnología y la programación! Tengo 24 años, me especializo en producción Front End, aunque he realizado cursos y carreras enfocadas a todas las ramas de la programación, capacitándome para convertirme en programador Full Stack. Siempre dispuesto a aprender nuevos conocimientos y dar lo mejor de sí mismo.",
  github: "https://github.com/matiasperdomo",
  resume:
    "https://www.linkedin.com/in/perdomomatias/",
  linkedin: "https://www.linkedin.com/in/perdomomatias/",
  twitter: "https://twitter.com/matiasperdomodj",
  insta: "https://www.instagram.com/matiasperdomodj/",
  facebook: "https://www.facebook.com/perdomomatias/",
};

const skillsHeader = {
  tittle:"Habilidades",
  description:"Idiomas y habilidades que he aprendido en los últimos 5 años."
};

const skills = [
  {
    title: "Frontend",
    skills: [
      {
        name: "React Js",
        image:
          "data:image/svg+xml;base64,",
      },
      {
        name: "Redux",
        image:
          "https://d33wubrfki0l68.cloudfront.net/0834d0215db51e91525a25acf97433051f280f2f/c30f5/img/redux.svg",
      },
      {
        name: "Next Js",
        image:
          "data:image/png;base64,",
      },
      {
        name: "Angular Js",
        image:
          "https://camo.githubusercontent.com/8886130b3d8aba95dbdd7c4f9a41029606424cc06d1873c1ced87dd55a222fef/68747470733a2f2f616e67756c61722e696f2f6173736574732f696d616765732f6c6f676f732f616e67756c61722f616e67756c61722e737667",
      },
      {
        name: "HTML",
        image: "https://www.w3.org/html/logo/badge/html5-badge-h-solo.png",
      },
      {
        name: "CSS",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/d/d5/CSS3_logo_and_wordmark.svg/1452px-CSS3_logo_and_wordmark.svg.png",
      },
      {
        name: "JavaScript",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6a/JavaScript-logo.png/800px-JavaScript-logo.png",
      },
      {
        name: "Bootstrap",
        image:
          "https://getbootstrap.com/docs/5.3/assets/brand/bootstrap-logo-shadow.png",
      },
      {
        name: "Material UI",
        image:
          "data:image/png;base64,",
      },
      {
        name: "Flutter",
        image:
          "https://cdn-images-1.medium.com/max/1200/1*5-aoK8IBmXve5whBQM90GA.png",
      },
    ],
  },
  {
    title: "Backend",
    skills: [
      {
        name: "Node Js",
        image: "https://nodejs.org/static/images/logo.svg",
      },
      {
        name: "Express Js",
        image:
          "data:image/png;base64,",
      },
      {
        name: "Graph Ql",
        image: "https://www.vectorlogo.zone/logos/graphql/graphql-icon.svg",
      },
      {
        name: "Python",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/python/python-original.svg",
      },
      {
        name: "Flask",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/3/3c/Flask_logo.svg/1280px-Flask_logo.svg.png",
      },
      {
        name: "Django",
        image:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSlFFyY16N5NRhzoG24RqB7x8Ok2t8Bdgs-tuMsOBXS2Q&s",
      },
      {
        name: "MySQL",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original-wordmark.svg",
      },
      {
        name: "Postgresql",
        image: "https://www.postgresql.org/media/img/about/press/elephant.png",
      },
      {
        name: "MongoDB",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/mongodb/mongodb-original-wordmark.svg",
      },
      {
        name: "Firebase",
        image: "https://www.vectorlogo.zone/logos/firebase/firebase-icon.svg",
      },
    ],
  },
  {
    title: "DevOps",
    skills: [
      {
        name: "AWS",
        image:
          "https://download.logo.wine/logo/Amazon_Web_Services/Amazon_Web_Services-Logo.wine.png",
      },
      {
        name: "Google Cloud",
        image:
          "https://static-00.iconduck.com/assets.00/google-cloud-platform-logo-icon-2048x1824-pg4wzspq.png",
      },
      {
        name: "Docker",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/docker/docker-original-wordmark.svg",
      },
    ],
  },
  {
    title: "Android",
    skills: [
      {
        name: "Java",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/java/java-original.svg",
      },
      {
        name: "Kotlin",
        image:
          "https://www.vectorlogo.zone/logos/kotlinlang/kotlinlang-icon.svg",
      },
      {
        name: "Jetpack Compose",
        image:
          "https://3.bp.blogspot.com/-VVp3WvJvl84/X0Vu6EjYqDI/AAAAAAAAPjU/ZOMKiUlgfg8ok8DY8Hc-ocOvGdB0z86AgCLcBGAsYHQ/s1600/jetpack%2Bcompose%2Bicon_RGB.png",
      },
      {
        name: "Android Studio",
        image:
          "https://developer.android.com/static/studio/images/new-studio-logo-1_1920.png",
      },
    ],
  },
  {
    title: "Programs",
    skills: [
      {
        name: "Adobe Photoshop",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/a/af/Adobe_Photoshop_CC_icon.svg/2101px-Adobe_Photoshop_CC_icon.svg.png",
      },
      {
        name: "Adobe Premiere",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/4/40/Adobe_Premiere_Pro_CC_icon.svg/2101px-Adobe_Premiere_Pro_CC_icon.svg.png",
      },
      {
        name: "GitHub Desktop",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/a/ae/Github-desktop-logo-symbol.svg/2048px-Github-desktop-logo-symbol.svg.png",
      },
      {
        name: "Resolume Arena",
        image:
          "https://resolume.com/gfx/press/Logos/Arena%20Logo.svg",
      },
    ],
  },
  {
    title: "Others",
    skills: [
      {
        name: "Git",
        image:
          "https://e7.pngegg.com/pngimages/713/558/png-clipart-computer-icons-pro-git-github-logo-text-logo-thumbnail.png",
      },
      {
        name: "GitHub",
        image:
          "https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png",
      },
      {
        name: "Netlify",
        image:
          "https://seeklogo.com/images/N/netlify-logo-BD8F8A77E2-seeklogo.com.png",
      },
      {
        name: "VS Code",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/9/9a/Visual_Studio_Code_1.35_icon.svg/512px-Visual_Studio_Code_1.35_icon.svg.png?20210804221519",
      },
      {
        name: "Slack",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/d/d5/Slack_icon_2019.svg/1024px-Slack_icon_2019.svg.png",
      },
      {
        name: "Discord",
        image:
          "https://www.svgrepo.com/show/353655/discord-icon.svg",
      },
      {
        name: "Figma",
        image:
          "https://s3-alpha.figma.com/hub/file/1481185752/fa4cd070-6a79-4e1b-b079-8b9b76408595-cover.png",
      },
      {
        name: "Wordpress",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/9/98/WordPress_blue_logo.svg/1024px-WordPress_blue_logo.svg.png",
      },
    ],
  },
];

const experienceHeader = {
  tittle:"Experiencia",
  description:"He trabajado en una amplia gama de proyectos. Desde aplicaciones web hasta aplicaciones de Android. Éstos son algunos de mis proyectos."
};

const experience = [
  {
    id: 0,
    img: "https://img.freepik.com/free-vector/creative-gradient-code-logo_23-2148820572.jpg?size=338&ext=jpg&ga=GA1.1.1448711260.1706745600&semt=ais",
    role: "Freelancer Web Designer",
    company: "Personal Business ",
    date: "2020 - 2024",
    desc: "A partir de lo aprendido durante estos años he decidido realizar trabajos de desarrollo web freelance.",
    skills: [
      "HTML",
      "CSS",
      "Javascript",
      "Bootstrap",
      "Wordpress",
      "React JS",
    ],
    doc: "https://htmlburger.com/blog/wp-content/uploads/2023/04/modern-website-design-examples.jpg",
  },
  {
    id: 1,
    img: "https://pbs.twimg.com/profile_images/717794779531255808/M7Af3wXB_400x400.jpg",
    role: "Desarrollador Web",
    company: "CBZ La Plata",
    date: "2018 - Oct 2024",
    desc: "Creación y mantenimiento de un sitio web en Wordpress. Luego implementación de ecommerce para la venta de materiales acústicos. Pagos procesados ​​a través de Mercado Pago.",
    skills: [
      "Wordpress",
      "Javascript",
      "Elementor Pro",
      "Woo Commerce",
    ],
    doc: "https://htmlburger.com/blog/wp-content/uploads/2023/04/modern-website-design-examples.jpg",
  },
];

const projectsHeader = {
  tittle:"Proyectos",
  description:"He trabajado en una amplia gama de proyectos. Desde aplicaciones web hasta aplicaciones de Android. Éstos son algunos de mis trabajos."
};

const projects = [
  {
    id: 11,
    title: "CBZ La Plata",
    date: "2020",
    description:
      "Creación y mantenimiento de un sitio web en Wordpress. Luego implementación de ecommerce para la venta de materiales acústicos. Pagos procesados ​​a través de Mercado Pago.",
    image:
      "https://raw.githubusercontent.com/matiasperdomo/portfolio-dev/main/src/images/projects/cbzlaplata.png",
    tags: [
      "WordPress",
      "JavaScript",
      "Elementor Pro",
    ],
    category: "web app",
    github: "",
    webapp: "https://cbzlaplata.com.ar/",
    member: [
      {
        name: "Matias Perdomo",
        img: "https://raw.githubusercontent.com/matiasperdomo/portfolio-dev/main/src/images/profile-image.jpg",
        linkedin: "https://www.linkedin.com/in/perdomomatias/",
        github: "https://github.com/matiasperdomo/",
      },
    ],
  },
  {
    id: 0,
    title: "Uga Love",
    date: "2021",
    description:
      "Tienda Online WooCommerce de productos de cosmetica, cuidado personal. Integracion con el Checkout de Mercado Pago y diseño con Elementor Pro.",
    image:
      "https://raw.githubusercontent.com/matiasperdomo/portfolio-dev/main/src/images/projects/ugalove.png",
    tags: [
      "WordPress",
      "JavaScript",
      "Elementor Pro",
      "WooCommerce",
      "MercadoPago Checkout",
    ],
    category: "web app",
    github: "",
    webapp: "https://ugalove.com.ar/",
    member: [
      {
        name: "Matias Perdomo",
        img: "https://raw.githubusercontent.com/matiasperdomo/portfolio-dev/main/src/images/profile-image.jpg",
        linkedin: "https://www.linkedin.com/in/perdomomatias/",
        github: "https://github.com/matiasperdomo/",
      },
    ],
  },
  {
    id: 0,
    title: "Chatty-Bot",
    date: "2023",
    description:
      "",
    image:
      "https://htmlburger.com/blog/wp-content/uploads/2023/04/modern-website-design-examples.jpg",
    tags: ["React Js", "MongoDb", "Node Js", "Express Js", "Redux"],
    category: "web app",
    github: "",
    webapp: "",
    member: [
      {
        name: "Matias Perdomo",
        img: "https://raw.githubusercontent.com/matiasperdomo/portfolio-dev/main/src/images/profile-image.jpg",
        linkedin: "https://www.linkedin.com/in/perdomomatias/",
        github: "https://github.com/matiasperdomo/",
      },
      {
        name: "Tomas Ruiz",
        img: "https://media.licdn.com/dms/image/C4D03AQG6As2_duzGOA/profile-displayphoto-shrink_800_800/0/1647382142613?e=2147483647&v=beta&t=MFjqE47tmFZJpKOKFsoUnCiIG_tioLx1q8OyDx4M29s",
        linkedin: "https://www.linkedin.com/in/tomas-isaias-ruiz-151350190/",
        github: "https://github.com/tomi-ruiz",
      },
    ],
  },
  {
    id: 1,
    title: "Mutual SPB",
    date: "2022",
    description:
      "Diseño de pagina estatica informativa con actualizaciones mensuales de valores, reintegros y otros servicios que ofrecen.",
    image:
      "https://raw.githubusercontent.com/matiasperdomo/portfolio-dev/main/src/images/projects/mutualspb.png",
    tags: [
      "React Js",
      "MongoDb",
      "Node Js",
      "Express Js",
      "Redux",
      "NodeMailer",
    ],
    category: "web app",
    github: "",
    webapp: "",
  },

  {
    id: 3,
    title: "Sistema de Venta - (C# / MySQL)",
    date: "2022",
    description:
      "El siguiente proyecto forma parte de un sistema de venta desarrollado en C# y vinculado a una base datos MySQL. Como ejemplo se tomo la marca HyperX, desarrolle un login, roles de usuarios, carga de productos, clientes, generación de ventas y posibilidad de exportar formato venta de tipo factura.",
    image:
      "https://r2.easyimg.io/fjpoismsz/captura_de_pantalla_2024-02-26_094958.png",
    tags: ["C#", "MySQL", "Visual Studio"],
    category: "others",
    github: "https://github.com/matiasperdomo/HyperX-SistemaVenta",
    webapp: "https://github.com/matiasperdomo/HyperX-SistemaVenta",
  },
  {
    id: 4,
    title: "Project Name",
    date: "2021",
    description:
      "A WhatsApp clone made with React JS, Firebase, and Material UI. It has Phone Authentication, Real-time Database. It has a chat room where users can chat with each other. It has a sidebar where users can see all the chat rooms and can create a new chat room. It has a login page where users can log in with their Google account.",
    image:
      "https://htmlburger.com/blog/wp-content/uploads/2023/04/modern-website-design-examples.jpg",
    tags: ["React Js", "Firebase", "Firestore", "Node JS"],
    category: "web app",
    github: "",
    webapp: "",
  },
  {
    id: 5,
    title: "Dashboard - Product Manager PHP",
    date: "2022",
    description:
      "Creación de panel de administración de productos para tienda web. Desarrollo de permisos, roles de usuarios, categorías de productos, etc.",
    image:
      "https://raw.githubusercontent.com/matiasperdomo/portfolio-dev/main/src/images/projects/hyperx3.png",
    tags: ["PHP", "MySQL", "XAMPP", "Bootstrap"],
    category: "web app",
    github: "http://hyperx.great-site.net/",
    webapp: "http://hyperx.great-site.net/",
  },
  {
    id: 6,
    title: "HyperX - Store / Cart",
    date: "2022",
    description:
      "Creación de sitio web en formato ecommerce con carrito, vinculado a base de datos MySQL",
    image:
      "https://raw.githubusercontent.com/matiasperdomo/portfolio-dev/main/src/images/projects/hyperx2.png",
    tags: ["HTML", "CSS", "Bootstrap", "JavaScript", "PHP", "MySQL"],
    category: "web app",
    github: "http://hyperx.great-site.net/",
    webapp: "http://hyperx.great-site.net/",
  },
];

const educationHeader = {
  tittle:"Educación",
  description:"Conoce mi trayectoria educativa en el mundo de la programación!"
};

const education = [
  {
    id: 6,
    img: "https://raw.githubusercontent.com/matiasperdomo/portfolio-dev/main/src/images/education/logo-coder.png",
    school: "CoderHouse",
    date: "2024",
    grade: "",
    desc: "React Native, Android Studio, Redux, Firebase",
    degree: "Desarrollo de aplicaciones",
  },
  {
    id: 7,
    img: "https://raw.githubusercontent.com/matiasperdomo/portfolio-dev/main/src/images/education/logo-davinci.png",
    school: "Escuela Da Vinci",
    date: "2021 - 2023",
    grade: "",
    desc: "Java, HTML5, CSS3, .NET, C++, C#, Javascript, Jquery, AngularJS, PHP, SQL Server, Oracle, MySQL, Linux, Android.",
    degree: "Analista de Sistemas",
  },
  {
    id: 8,
    img: "https://raw.githubusercontent.com/matiasperdomo/portfolio-dev/main/src/images/education/logo-educacionit.png",
    school: "Educacion IT",
    date: "2021 - 2024",
    grade: "",
    desc: "Desarrollo Web con HTML, Introducción a UX, Javascript desde cero, Maquetador Web Avanzado, Introducción al Paradigma de Objetos, Javascript Desarrollador Avanzado, Responsive Web & Bootstrap, Git: Desarrollo Colaborativo, React.JS Developer, Javascript AWP, Presupuesto Web",
    degree: "Front End Web Developer",
  },
  {
    id: 9,
    img: "https://raw.githubusercontent.com/matiasperdomo/portfolio-dev/main/src/images/education/logo-informatica.png",
    school: "Facultad de Informatica - UNLP",
    date: "2018 - 2020",
    grade: "",
    desc: "I started my bachelor's degree in systems, although I did not finish it.",
    degree: "Licenciatura en Informatica",
  },
  {
    id: 10,
    img: "https://raw.githubusercontent.com/matiasperdomo/portfolio-dev/main/src/images/education/logo-bivongi.png",
    school: "Escuela Instituto Bivongi",
    date: "2012 - 2017",
    grade: "",
    desc: "Colegio ubicado en la ciudad de La Plata. Aquí terminé mi educación secundaria.",
    degree: "Educación Secundaria",
  },
];

const contact = {
  tittle: "Contacto",
  description: "No dudes en comunicarte conmigo si tienes alguna pregunta u oportunidad!",
  tittle2: "Enviame un email",
  email: "Tu correo",
  name: "Tu nombre",
  subject: "Asunto",
  message: "Mensaje",
  send: "Enviar",
};

const footer = {
  about: "Bio",
  skills: "Habilidades",
  experience: "Experiencia",
  projects: "Proyectos",
  education: "Educación",
  contact: "Contacto",
  copyright: "2024 Matias Perdomo. Todos los derechos reservados",
};

const constants = {
  nav,
  hero,
  bio,
  skillsHeader,
  skills,
  experienceHeader,
  experience,
  projectsHeader,
  projects,
  educationHeader,
  education,
  contact,
  footer,
};

export default constants;